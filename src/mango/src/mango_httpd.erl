...
handle_req_int(#httpd{path_parts=[_, <<"_explain">> | _]} = Req, Db) ->
    fabric2_fdb:transactional(Db, fun (TxDb) ->
        handle_explain_req(Req, TxDb)
    end);
handle_req_int(#httpd{path_parts=[_, <<"_find">> | _]} = Req, Db) ->
    fabric2_fdb:transactional(Db, fun (TxDb) ->
        handle_find_req(Req, TxDb)
    end);
handle_req_int(_, _) ->
    ...
    Status = fabric2_fdb:transactional(Db, fun (TxDb) ->
        ...
        case mango_idx:add(DDoc, Idx) of
            ...
            {ok, NewDDoc} ->
                ...
                case mango_crud:insert(TxDb, NewDDoc, CreateOpts) of
                    {ok, [{RespProps}]} ->
    ...
    {Success, Fail} = fabric2_fdb:transactional(Db, fun (TxDb) ->
        ...
        DelOpts = get_idx_w_opts(Opts),
         lists:foldl(fun(DDocId0, {Success0, Fail0}) ->
            ...
            Id = {<<"id">>, DDocId},
            case mango_idx:delete(Filt, TxDb, Idxs, DelOpts) of
                {ok, true} ->
...
handle_index_req(#httpd{method='DELETE',
    ...
    Result = fabric2_fdb:transactional(Db, fun (TxDb) ->
        ...
        mango_idx:delete(Filt, TxDb, Idxs, DelOpts)
    end),
    case Result of
        {ok, true} ->
